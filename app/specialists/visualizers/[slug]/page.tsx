        ...
        <div className="lg:col-span-3">
          <div className="sticky top-24 space-y-6">
            <div className="flex flex-col gap-8 mb-6">
              <Image
                src={visualizer.avatar || '/user.svg'}
                width={80}
                height={80}
                alt={visualizer.name}
                className={`w-20 h-20 rounded-full ${visualizer.avatar ? 'object-cover' : 'object-none'}`}
                priority
              />
            ...
            {projects.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects.map((project) => (
                  <ProjectCard key={project.id} project={project} category="visualizers" />
                ))}
              </div>
            ) : (
